...
// The extension can be reloaded (chrome://extensions → ↻) while an old tab is
// still open. After a reload, the stale script's chrome.* calls throw
// *synchronously* with "Extension context invalidated" — a `.catch()` on the
// returned promise does not catch a synchronous throw, which is how it
// surfaced as an uncaught error. Route every chrome call through this helper:
// the throw is swallowed, and once invalidated the bridge stops relaying (the
// page is stale; the reloaded extension injects a fresh bridge on refresh).
let invalidated = false;
function chromeTry(fn) {
...
window.addEventListener('message', async (ev) => {
  ...
  try {
    if (op === 'get') {
      const obj = await chromeTry(() => chrome.storage.local.get(key));
      window.postMessage({ psa: RES, id, ok: true, value: obj?.[key] }, '*');
    } else if (op === 'set') {
      await chromeTry(() => chrome.storage.local.set({ [key]: value }));
...
window.addEventListener('message', (ev) => {
  ...
  chromeTry(() => chrome.runtime.sendMessage({ psa: 'psa-badge', state: ev.data.state }))?.catch?.(() => {});
});
...
window.addEventListener('message', (ev) => {
  ...
  chromeTry(() =>
    chrome.runtime.sendMessage({ psa: 'psa-capture-status', ok: !!ev.data.ok, error: ev.data.error ?? null })
  )?.catch?.(() => {});
});
...
window.addEventListener('message', (ev) => {
  ...
  chromeTry(() => chrome.runtime.sendMessage({ psa: 'psa-ocr-req', id, width, height, pixels: plain }))
    ?.then((res) => {
